...
type Props = {
  ...
  courseId: string;
  courseTitle: string;
  // Called when timer fires completion — caller provides current progress
...
export function PlaybookTimer({ userId, courseId, courseTitle, onComplete }: Props) {
  ...
  const handleStart = useCallback(() => {
    ...
    localStorage.setItem(key, JSON.stringify({ endsAt, hours: pickedHours }));
    ...
    return (
      <div style={{
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1d4ed8", marginBottom: 10 }}>
          ⏱️ Select Timer Duration
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          {HOURS.map((h) => (
            <button
              key={h}
              onClick={() => setPickedHours(h)}
              style={{
                padding: "8px 18px", borderRadius: 6, border: "2px solid",
                borderColor: pickedHours === h ? "#2563eb" : "#d1d5db",
                background: pickedHours === h ? "#2563eb" : "#fff",
                color: pickedHours === h ? "#fff" : "#374151",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            onClick={handleStart}
            disabled={!pickedHours}
            style={{
              padding: "7px 18px", borderRadius: 6,
              background: pickedHours ? "#2563eb" : "#93c5fd",
              color: "#fff", border: "none",
              fontSize: 13, fontWeight: 600,
              cursor: pickedHours ? "pointer" : "not-allowed",
            }}
          >
            Confirm & Start
          </button>
  ...
  if (state === "running") {
    ...
    return (
      <div style={{
        ...
        border: "1px solid #86efac", borderRadius: 8, marginBottom: 12,
      }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: "#15803d", fontVariantNumeric: "tabular-nums" }}>
                {fmt(remaining)}
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{
      ...
      border: "1px solid #fcd34d", borderRadius: 8, marginBottom: 12,
      display: "flex", alignItems: "center", gap: 10,
    }}>
      <span style={{ fontSize: 18 }}>🏁</span>
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 600, fontSize: 13, color: "#92400e" }}>
          Timer Complete! {sending ? "Sending summary email..." : "Summary email sent."}
        </div>
      </div>
      <button
        onClick={handleCancel}
