...
type AiBot = { id: string; name: string; assignedRoles: string[] };
...
export function BotChatWidget({ role }: { role: string }) {
  ...
  async function send() {
    if ((!input.trim() && attachments.length === 0) || loading || !selectedBot || !user) return;
    ...
    try {
      ...
      const assistantMsg: Message = { role: "assistant", content: data.message || "Sorry, I couldn't respond.", timestamp: new Date().toISOString() };
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 120px)", background: "#fff", borderRadius: "12px", border: "1px solid #e5e7eb", overflow: "hidden", position: "relative" }}>
      ...
      {/* ── Left Sidebar ── */}
      <div
        className={`chat-sidebar${sidebarCollapsed ? " chat-sidebar-hidden" : ""}`}
        style={{
          width: "260px",
          minWidth: "260px",
          background: "#1a1a2e",
        ...
        <div style={{ padding: "16px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          {bots.length > 1 && (
            ...
            >
              {bots.map(b => <option key={b.id} value={b.id} style={{ background: "#1a1a2e" }}>{b.name}</option>)}
            </select>
          )}
          <button onClick={startNewChat} style={{
            width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.1)", color: "#fff",
            border: "1px solid rgba(255,255,255,0.2)", borderRadius: "8px", fontSize: "13px", fontWeight: 600,
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
          {chatSessions.length === 0 ? (
            ...
            chatSessions.map(session => (
              <div
                key={session.chatId}
                onClick={() => { loadSession(session); setSidebarCollapsed(true); }}
                style={{
                  padding: "10px 12px", borderRadius: "8px", cursor: "pointer", marginBottom: "2px",
                  background: currentChatId === session.chatId ? "rgba(255,255,255,0.15)" : "transparent",
              ...
              >
                ...
                <button
                  onClick={e => deleteSession(session.chatId, e)}
                  style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: "14px", padding: "2px 4px", flexShrink: 0 }}
                >
                  🗑
                </button>
              </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#fff" }}>
        ...
        {/* Top bar */}
        <div style={{ padding: "12px 20px", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
          <button
          ...
          <div style={{ fontWeight: 600, fontSize: "15px", color: "#1f2937" }}>🤖 {selectedBot?.name}</div>
        </div>

        {/* Messages */}
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 0" }}>
          {isNewChat ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "#9ca3af", textAlign: "center", padding: "40px" }}>
              <div style={{ fontSize: "56px", marginBottom: "16px" }}>🤖</div>
              <div style={{ fontSize: "22px", fontWeight: 700, color: "#1f2937", marginBottom: "8px" }}>How can I help you today?</div>
              <div style={{ fontSize: "14px", color: "#6b7280" }}>Ask me anything — I'm trained and ready.</div>
            </div>
          ) : (
            <div style={{ maxWidth: "760px", margin: "0 auto", padding: "0 24px", display: "flex", flexDirection: "column", gap: "24px" }} className="chat-messages-inner">
              {messages.map((m, i) => (
                <div key={i} style={{ display: "flex", gap: "14px", alignItems: "flex-start", flexDirection: m.role === "user" ? "row-reverse" : "row" }}>
                  {/* Avatar */}
                  <div style={{
                    width: 36, height: 36, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px",
                  ...
                  <div style={{ flex: 1, maxWidth: "85%" }}>
                    ...
                    {m.attachments && m.attachments.length > 0 && (
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                        {m.attachments.map((att, ai) => (
                          <div key={ai} style={{ padding: "6px 12px", background: "#f3f4f6", borderRadius: "8px", fontSize: "12px", color: "#374151", display: "flex", alignItems: "center", gap: "6px" }}>
                            📎 {att.name}
                          </div>
                        ))}
                      </div>
                    ...
                    {m.content && (
                      <div style={{
                        ...
                        background: m.role === "user" ? "#1f2937" : "#f9fafb",
              ...
              {loading && (
                <div style={{ display: "flex", gap: "14px", alignItems: "flex-start" }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#f3f4f6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }}>🤖</div>
                  <div style={{ padding: "12px 16px", borderRadius: "18px 18px 18px 4px", background: "#f9fafb", border: "1px solid #e5e7eb" }}>
                    <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
                      {[0, 1, 2].map(i => (
                        <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#9ca3af", animation: `bounce 1.2s ${i * 0.2}s infinite` }} />
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {attachments.map((att, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 10px", background: "#f3f4f6", borderRadius: "8px", fontSize: "12px" }}>
                  {att.type.startsWith("image/") ? (
                    <img src={att.url} alt={att.name} style={{ width: 40, height: 40, objectFit: "cover", borderRadius: "6px" }} />
                  ) : (
                    <span>📎</span>
                  )}
          ...
          <div style={{ maxWidth: "760px", margin: "0 auto", position: "relative" }} className="chat-input-inner">
            ...
            {showAttachMenu && (
              <div ref={attachMenuRef} style={{
                ...
                background: "#2a2a3e", borderRadius: "14px", padding: "8px",
                boxShadow: "0 8px 32px rgba(0,0,0,0.3)", zIndex: 100, minWidth: "220px",
                border: "1px solid rgba(255,255,255,0.1)"
              }}>
                {[
                  { icon: "🖼️", label: "Upload photos & files", accept: "image/*,.pdf,.docx,.doc,.txt,.csv,.xlsx", ref: fileRef },
                ].map((item, i) => (
                  <button
                    key={i}
                    ...
                    style={{
                      ...
                      color: "#fff", fontSize: "14px", borderRadius: "8px", textAlign: "left"
                    }}
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: "10px", background: "#f9fafb", borderRadius: "14px", border: "1px solid #e5e7eb", padding: "8px 12px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: showAttachMenu ? "#1f2937" : "#e5e7eb",
                  color: showAttachMenu ? "#fff" : "#374151",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: "20px", fontWeight: 300, flexShrink: 0, lineHeight: 1, transition: "background 0.15s"
                }}
                title="Attach"
              >
                +
              ...
              <textarea
                ...
                rows={1}
                style={{
                  flex: 1, border: "none", outline: "none", background: "transparent", fontSize: "14px",
                  resize: "none", lineHeight: "1.6", fontFamily: "inherit", height: "52px",
                  maxHeight: "200px", overflowY: "auto", padding: "8px 0", color: "#1f2937"
                }}
              />
              ...
              <button
                ...
                style={{
                  ...
                  background: loading || (!input.trim() && attachments.length === 0) ? "#e5e7eb" : "#1f2937",
                  color: loading || (!input.trim() && attachments.length === 0) ? "#9ca3af" : "#fff",
                  display: "flex", alignItems: "center", justifyContent: "center",
...
const centerStyle: React.CSSProperties = {
  ...
  minHeight: "400px", textAlign: "center", color: "#9ca3af"
};
