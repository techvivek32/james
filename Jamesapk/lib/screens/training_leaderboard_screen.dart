...
class _TrainingLeaderboardScreenState extends State<TrainingLeaderboardScreen> {
  ...
  String _lbCacheKey(dynamic course) => 'lb_cache_${course?['id']}';
  ...
  Future<void> _init() async {
    ...
    // Cache-first: paint the picker + last-known leaderboard instantly from local
    // storage so the page is never stuck on a spinner while the network runs.
    final hadCache = await _loadCachedCourses();
    await _fetchCourses(triggerLeaderboard: !hadCache);
  }

  // Reuse the course list already cached by the Courses screen ('courses_cache')
  // — no need to wait on a fresh /api/courses call just to fill the picker.
  // Returns true if a cached course was shown.
  Future<bool> _loadCachedCourses() async {
    try {
      ...
      if (_selectedCourse != null) {
        await _loadCachedLeaderboard(_selectedCourse); // instant rows
        _fetchLeaderboard(_selectedCourse);            // refresh in background
      }
      return true;
    } catch (e) {
  ...
  Future<void> _fetchCourses({bool triggerLeaderboard = true}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Only kick a leaderboard fetch here when the cache path didn't already
        // (avoids a duplicate request on every open).
        if (triggerLeaderboard && _selectedCourse != null) {
  ...
  Future<void> _fetchLeaderboard(dynamic course) async {
    // Keep any cached rows visible; only show the spinner when we have nothing.
    setState(() {
    ...
    try {
      ...
      final leaderboardResponse = await api
                    ...
                    return ListTile(
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          _leaderboardRows = []; // drop the previous course's rows
                        });
                        Navigator.pop(context);
                        // Show this course's cached board instantly, then refresh.
                        _loadCachedLeaderboard(course);
